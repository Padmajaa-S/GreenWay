...
const Returns = () => {
  ...
  useEffect(() => {
    const queryOrders = searchParams.get("orders");
    if (queryOrders) {
      try {
        const parsedOrders = JSON.parse(decodeURIComponent(queryOrders));
        setReturnOrders([parsedOrders]);
      } catch (error) {
        console.error("Invalid order data in URL:", error);
      }
    }
  }, [searchParams]);
  ...
  <table className="min-w-full divide divide-black table-fixed">
    ...
    <tbody className="bg-white divide-y divide-gray-200">
      {warehouseAddresses
        ...
        .map((address) => {
          // Find the route time for this address
          ...
          {stats.map((item, index) => (
            <div key={index} className="w-32 flex flex-col items-center rounded-lg overflow-hidden shadow-md border-green-800 relative group">
              <div className="bg-green-200 w-full flex flex-col items-center justify-center p-3 border-green-800 border-b-4">
                ...
                {/* Display the question mark icon only for item name "cost" */}
                {item.name === "Cost" && (
                  <div className="absolute top-2 right-2">
                    {/* Lucide Question mark icon */}
